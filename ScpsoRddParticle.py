...
spark = SparkSession.builder.appName('Parallel_SCPSO').getOrCreate()
...
# Parallel PSO update equations
def parallel_update_velocity_position(particle, gbest_centroids):
...
def parallel_pso():
    ...
    for iteration in range(max_iterations):
        ...
        # Perform parallel update on each particle
        particles_rdd.foreach(lambda particle: parallel_update_velocity_position(particle, gbest_centroids))

        # Collect updated particles back to the driver program
        particles = particles_rdd.collect()

    return gbest_centroids

# Example usage
start_time = time.time()
best_centroids = parallel_pso()
end_time = time.time()
